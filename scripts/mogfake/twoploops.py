...
def noiseloop(sigma, model, params, pnames, M=20, deglist=range(5,20), **kwargs):
    iprint = kwargs.pop('iprint', 0)
    delta = kwargs.pop('delta', 1E-5)
    lamb = kwargs.pop('lamb', .1)
    ...
    for i in range(M):
        ...
        sr_results.append(srmog.fit(pnames, p0, delta=1E-6, iprint=iprint)[:2])

        fshift, fsrecon = standardmethod(data)
        fsmog = makemog(fsrecon, sigma, *params)
        fs_results.append(fsmog.fit(pnames, p0, delta=1E-6, iprint=iprint)[:2])
...
if __name__ == '__main__':
    #Experimental setup
    M = 50  # number of noise instances
    Nsigma = 10  # number of noise strengths
    ...
    truep = tmog.fit(pnames, p0, delta=1E-6, iprint=1)
    ...
    for i, sigma in enumerate(sigmalist):
        ...
        deglist = range(maxdeg-2, maxdeg+3) if i else range(5, 20)

        maxdeg, sr_results, fs_results = noiseloop(
            sigma, model, params, pnames=pnames, M=M, deglist=deglist
        )
